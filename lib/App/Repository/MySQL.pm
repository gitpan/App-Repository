...
######################################################################
## File: $Id: MySQL.pm 9232 2007-03-12 19:58:23Z spadkins $
######################################################################

use App::Repository::DBI;

package App::Repository::MySQL;
$VERSION = (q$Revision: 9232 $ =~ /(\d[\d\.]*)/)[0];  # VERSION numbers generated by svn
...
sub import_rows {
    ...
    if ($import_method eq "basic") {
        ...
        if ($debug_sql) {
            $elapsed_time = $self->_read_timer($timer);
            print "DEBUG_SQL: import_rows=[$nrows] ($elapsed_time sec) $DBI::errstr : $@\n";
        }
        die $@ if ($@);
    }
